...
(function($) {
    ...
    $(document).ready(function () {
        ...
        $('#palleon').palleon({
            baseURL: "https://app.viraldashboard.io/deziner_new/design/", // The url of the main directory. For example; "http://www.mysite.com/palleon-js/"
            ...
            watermarkText: 'https://app.viraldashboard.io/', // The watermark text
            ...
            customFunctions: function(selector, canvas, lazyLoadInstance) {
                ...
                selector.find('.template-grid').on('click','.template-favorite button.star',function(){
                    ...
                    toastr.success("We are working on it..", "Info");
                ...
                selector.find('.palleon-frames-grid').on('click','.frame-favorite button.star',function(){
                    ...
                    var frameid = button.data('frameid'); 
                         $.ajax({
                            ...
                            data:{ 'action':'Frame','frame':frameid}, 
                            success:function(response){  
                                 if(response == 1){
                                   toastr.success("Frame Added to Favorite", "Info");
                                 }
                               }
                        }); 
                });
                ...
                selector.find('.palleon-grid').on('click','.element-favorite button.star',function(){
                    ...
                    var elementid = button.data('elementid');
                     ...
                     $.ajax({
                            type:'POST',
                            url:'add-Favorite.php',
                            data:{ 'action':'Element','element':elementid}, 
                            success:function(response){  
                                 if(response == 1){
                                   toastr.success("Element Added to Favorite", "Info");
                                 }
                               }
                        }); 
            ...
            //////////////////////* SAVE TEMPLATE *//////////////////////
            saveTemplate: function(selector, template,imgData) {
                /**
                ...
                $('#palleon-main-loader').css("display","block");
                      
                $.ajax({
                type:'POST',
                url:'save-state.php',
                data:{ 'image':imgData,'template':template,'token':token_,'cat_id':cat_id,'sub_id':sub_id,'template_name':template_name}, 
                success:function(response){ 
                     $('#palleon-main-loader').css("display","none");
                    //if(response == 1){
                       toastr.success("Template Saved", "Info");
                   // }
                   }
               });
                /* Do what you want */
